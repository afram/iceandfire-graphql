...
function objectUrl(id, type) {
  return "".concat(prefix).concat(urlTypes[type], "/").concat(id, "/");
}
/**
...
function pageUrl(type, objects, num) {
  ...
  var url = "".concat(prefix).concat(urlTypes[type], "/");
...
function makePage(type, objects, num) {
  return {
    ...
    results: Object.keys(objects).slice((num - 1) * 10, num * 10).map(function (objectId) {
...
function addTypeToCache(cache, type, objects) {
  Object.keys(objects).forEach(function (objectId) {
    cache[objectUrl(objectId, type)] = JSON.stringify(formatObject(objects[objectId], type, objectId));
  });

  for (var i = 1; !isInvalidPage(objects, i); i += 1) {
...
function getType(fixture) {
  ...
  throw new Error("unable to match type for fixture: ".concat(JSON.stringify(fixture)));
}
      ...
      if (Array.isArray(object[key])) {
        object[key].forEach(function (foreignId) {
          if (objects[foreignType][foreignId]) {
            ...
            objects[foreignType][foreignId][foreignFieldName].push(parseInt(objectId, 10));
          }
        });
      } else {
...
function githubUrlForType(type) {
  return "https://raw.githubusercontent.com/afram/AnApiOfIceAndFire/master/AnApiOfIceAndFire.Data.Feeder/Data/".concat(type, ".json");
}
/**
...
var types = ['books', 'characters', 'houses'];
Promise.all(types.map(githubUrlForType).map(fetchFromUrl)).then(function (fixturesList) {
  var _ref;

  var fixtures = (_ref = []).concat.apply(_ref, (0, _toConsumableArray2["default"])(fixturesList));

  var objects = getObjectsFromFixtures(fixtures);
  populateReverseMaps(objects);
  var cache = getCacheFromObjects(objects);
  console.log("/* Generated by combine.js */\n         \n        var data = ".concat(JSON.stringify(cache, null, 2), ";\n        \n        export default data;"));
})["catch"](function (err) {
  console.error(err);
});